...
/*
 Tournament Winner
   ...
   respectively. The competitions array has elements in the form of [homeTeam, awayTeam] , where each team isa string of at most
   ...
   competition in the competitions array. Specifically. results [i] denotes the winner of competitions [i] . where a 1 in the
   results array means that the home team in the corresponding competition won anda o means that the away team won.
...
{
    //O(N)
    public static string TournamentWinnerFast(
        List<List<string>> competitions, List<int> results)
    {
        ...
        {
            var index = results[count] == 1 ? 0 : 1;
            var roundWinner = competition[index];
            var newScore = 0;

            if (!result.TryAdd(roundWinner, 3))
            {
                result.TryGetValue(roundWinner, out var score);
                newScore = score + 3;

                result[roundWinner] = newScore;
            }

            if (newScore >= maxScore || maxScore == 0)
            {
                winner = roundWinner;
                maxScore = newScore;
            }
