...
public static class LongestPeak
{
     ...
     *Longest Peak
       ...
       A peak is defined as adjacent integers in the array that are strictly increasing until they reach a tip (the highest value in the peak), at
       ...
       Similarly. the integers 1, 2, 3 don't form a peak because there aren't any strictly decreasing integers after the 3.
        ...
        Expected output: 6 //0, 10, 6, 5, -1, -3
     */
    public static int LongestPeakFast(int[] array)
    {
        var result = 0;
        if (array.Length < 3)
        {
        ...
        for (var i = 1; i < array.Length - 1; i++)
        {
            if (array[i] > array[i - 1] && array[i] > array[i + 1])
            {
                ...
                result = Math.Max(result, currentPeak);
            }
        }
