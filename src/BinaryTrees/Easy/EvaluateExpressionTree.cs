...
public static class EvaluateExpressionTree
{
    /*
     *Evaluate Expression Tree
       You're given a binary expression tree. Write a function to evaluate this tree mathematically and return a single resulting
       integer.
       ...
       the bottom of the tree is always evaluated first, regardless of the operator.
        ...
        Sample Output: 6 // ((2 * 3) - 2) + (8 / 3)
     */
    public static int GetEvaluateExpressionTree(BinaryTree tree)
    {
        return EvaluateTree(tree);
    }

    private static int EvaluateTree(BinaryTree node)
    {
        if (node.value < 0)
        {
